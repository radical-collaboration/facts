...
def ipccar6_postprocess_icesheet(locationfilename, pipeline_id):
	...
	# Write to netcdf
	writeNetCDF(gissl, pipeline_id, "GIS", targyears, site_lats, site_lons, site_ids, scenario, model_driver)
	writeNetCDF(waissl, pipeline_id, "WAIS", targyears, site_lats, site_lons, site_ids, scenario, model_driver)
	writeNetCDF(eaissl, pipeline_id, "EAIS", targyears, site_lats, site_lons, site_ids, scenario, model_driver)
	writeNetCDF(eaissl + waissl, pipeline_id, "AIS", targyears, site_lats, site_lons, site_ids, scenario, model_driver)


def writeNetCDF(data, pipeline_id, icesheet_name, targyears, site_lats, site_lons, site_ids, scenario, model_driver):
	...
	# Assign attributes
	rootgrp.description = "Local SLR contributions from icesheets according to ISMIP emulated workflow"
	rootgrp.history = "Created " + time.ctime(time.time())
	rootgrp.source = "SLR Framework: IPCCAR6 ISMIP emulated workflow - {0} - {1}".format(scenario, model_driver)
...
if __name__ == '__main__':
	...
	ipccar6_postprocess_icesheet(args.locationfile, args.pipeline_id)
