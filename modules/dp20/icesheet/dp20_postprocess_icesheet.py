...
''' dp20_postprocess_icesheet.py

This script runs the ice sheet post-processing task for the DP20 workflow. This task
uses the global projections from the 'dp20_project_icesheet' script and applies
...
def dp20_postprocess_icesheet(locationfilename, pipeline_id):
...
def writeNetCDF(data, pipeline_id, icesheet_name, targyears, site_lats, site_lons, site_ids, scenario):
	...
	# Assign attributes
	rootgrp.description = "Local SLR contributions from {} icesheet according to DP20 workflow".format(icesheet_name)
	rootgrp.history = "Created " + time.ctime(time.time())
	rootgrp.source = "SLR Framework: DP20 workflow"
...
if __name__ == '__main__':
	...
	parser = argparse.ArgumentParser(description="Run the post-processing stage for the DP20 icesheet SLR projection workflow",\
	...
	dp20_postprocess_icesheet(args.locationfile, args.pipeline_id)
