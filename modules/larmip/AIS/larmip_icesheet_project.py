...
def larmip_project_icesheet(pipeline_id, nsamps, targyears, baseyear, seed, models, cyear_start, cyear_end):
    ...
    # Create the sample pool from 0 -> nsamps
    ...
    for model_idx, this_model in enumerate(models):
        ...
        # Randomly pulls indices from time_Series_idx equal to the samps_per_model for this model
        rnd_sample_idx = rng.choice(time_series_idx,
                                    size=samps_per_model[model_idx],
                                    replace=False)

        # Remove the indices being sampled from the available time_series_indices
        time_series_idx = np.setdiff1d(time_series_idx, rnd_sample_idx)

        # Loop over the number of samples for this model
        for temp_idx in rnd_sample_idx:
...
if __name__ == "__main__":
    ...
    parser.add_argument('--models', help="List of model names to include in the sampling process", nargs="+",
                        default=None)
    ...
    parser.add_argument('--cyear_start', help="Constant rate calculation for projections starts at this year",
                        default=None, type=int)
    parser.add_argument('--cyear_end', help="Constant rate calculation for projections ends at this year", default=None,
                        type=int)
    ...
    larmip_project_icesheet(nsamps=args.nsamps, pipeline_id=args.pipeline_id, cyear_start=args.cyear_start,
                            cyear_end=args.cyear_end, targyears=targyears, seed=args.seed, baseyear=args.baseyear,
                            models=args.models)
