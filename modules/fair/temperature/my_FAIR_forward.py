...
from fair.forcing import (ozone, ozone_tr, ozone_st, h2o_st, contrails, aerosols,
    bc_snow, landuse)
from fair.gas_cycle.gir import calculate_alpha, step_concentration
from fair.gas_cycle.fair1 import carbon_cycle
from fair.forcing.ghg import co2_log, minor_gases
...
def fair_scm(
    ...
    if temperature_function=='Millar':
        from fair.temperature.millar import forcing_to_temperature
    elif temperature_function=='Geoffroy':
        from fair.temperature.geoffroy import forcing_to_temperature
    else:
    ...
    if ariaci_out:
        if temperature_function=='Geoffroy':
            return C, F, T, Tdeep, ariaci, lambda_eff, ohc, heatflux
        else:
            return C, F, T, ariaci
    else:
        if temperature_function=='Geoffroy':
            if gir_carbon_cycle:
                return C, F, T, Tdeep, lambda_eff, ohc, heatflux, airborne_emissions/cumulative_emissions
            else:
                return C, F, T, Tdeep, lambda_eff, ohc, heatflux
        else:
            return C, F, T
